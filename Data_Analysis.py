###  THIS IS A LIBRARY OF FUNCTIONS  FOR CANOPY LAB'S
...
#   This is a dictionary of reverse scores, that scoring scripts
#   can reference when tallying items in a questionnaire, followed by a dictionary
#   containing a rubric for how to reverse score the items, followed by how many total questions
#   there are in the questionnaire. MASQ is 39 question version with AD-22 and AA-17 subscales.
#
#   An example to illustrate each item in dictionary. Look at the first line. This pertains
#   to the TMMS questionnaire. The 3 keywords inside the TMMS key pertain to 1. Reverse Scored Items,
#   2. Reverse scoring rubric and 3. Number of questions in questionnaire.
#
...
def get_questionnaire_info():
        info_dict={'TMMS':[[2, 4, 5, 8, 15, 16, 17, 19, 22, 23, 24, 27, 28, 30], {1:5,2:4,4:2,5:1}, 30],
                            ...
                            'PANAS_TRAIT':[[],{},20],
                            'PANAS_TRAIT_26':[[],{},26], #JOEL SCHEMA STUDY
                            'PANAS_TRAIT_31':[[],{},31], #JOEL SCHEMA STUDY
                            'PANAS_STATE_31':[[],{},31], #JOEL SCHEMA STUDY
                            'PANAS_CHILD':[[],{},10],#JOEL SCHEMA STUDY
                            'YSQ':[[],{},25], #JOEL SCHEMA STUDY
                            'RELATIONS_1':[[],{},1], #JOEL SCHEMA STUDY
                            'RELATIONS_2':[[],{},4], #JOEL SCHEMA STUDY
                            'TAT_18':[[],{},18], #JOEL SCHEMA STUDY
                            'TAT_SET':[[],{},1], #JOEL SCHEMA STUDY
                            'TAT_7':[[],{},7], #JOEL SCHEMA STUDY
                            'TAT_CODER':[[],{},1], #JOEL SCHEMA STUDY
...
def deidentifier(filename,colheaders):
    ...
    with open(filename, 'rU') as f:
        r = csv.reader(f)
        lines = [line for line in r]
        col_name_index = int(raw_input('What column index (i.e. column-1) are the participants naemes?'))
        name = filename[:-4]
